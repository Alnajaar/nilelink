...
export class Database {
  static async getClient() {
    try {
      ...
      // Return a mock client for build safety if DB is unavailable
      return { query: async () => ({ rows: [] }), release: () => { } } as any;
    }
  }
  ...
  static async updateRestaurant(id: number, updates: any) {
    return null;
  }
  ...
  static async getChainByRestaurantId(id: number) {
    return null;
  }
}