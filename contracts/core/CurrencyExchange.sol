    ...
    ReentrancyGuard
{
    ...
    mapping(address => mapping(bytes3 => NileLinkTypes.RateSnapshot))
        public restaurantRates;
    ...
    function snapshotRate(
        ...
        override
        nonReentrant
        returns (NileLinkTypes.RateSnapshot memory snapshot)
    {
    ...
    function _getExpectedRange(
        bytes3 currency
    ) internal pure returns (string memory range) {
        if (currency == "LBP") {
    ...
    function _pruneRateHistory(bytes3 currency) internal {
        ...
        if (removeCount > 0) {
            ...
            // Resize array
            for (
                uint256 i = history.length - removeCount;
                i < history.length;
                i++
            ) {
                history.pop();
