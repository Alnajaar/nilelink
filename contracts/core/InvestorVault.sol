...
contract InvestorVault is IInvestorVault, Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(
        ...
        RestaurantInvestment storage restInvestment = restaurantInvestments[
            restaurant
        ];
        Investment storage investorInvestment = investments[msg.sender][
            restaurant
        ];

        // Calculate new ownership percentage
        uint256 newTotalInvested = restInvestment.totalInvestedUsd6 +
            amountUsd6;
    ...
    function withdraw(
        ...
        override
        nonReentrant
        onlyValidInvestment(msg.sender, restaurant)
        whenNotPaused
    {
        Investment storage investorInvestment = investments[msg.sender][
            restaurant
        ...
        uint256 ownershipToRemove = (amountUsd6 * 10000) /
            restInvestment.totalInvestedUsd6;
        ...
        uint256 accruedDividends = calculateAccruedDividends(
            msg.sender,
            restaurant
        );
        ...
        returns (uint256 claimedUsd6)
    {
        ...
        if (claimedUsd6 > 0) {
            ...
            investorInvestment.lastDividendCalculation = uint64(
                block.timestamp
            );
            ...
            dividendHistory[msg.sender].push(
                DividendPayment({
    ...
    function calculateAccruedDividends(
        ...
        Investment storage investorInvestment = investments[investor][
            restaurant
        ];
        RestaurantInvestment storage restInvestment = restaurantInvestments[
            restaurant
        ...
        uint256 investorShare = (restInvestment.netProfit *
            investorInvestment.ownershipBps) / 10000;
    ...
    function positionOf(
        ...
        view
        override
        returns (uint256 investedUsd6, uint256 ownershipBps)
    {
    ...
    function getPortfolio(
        address investor
    ) external pure returns (Portfolio memory portfolio) {
    ...
    function updateRestaurantValuation(
        ...
        restInvestment.netProfit = newValuation >
            restInvestment.totalInvestedUsd6
            ? newValuation - restInvestment.totalInvestedUsd6
            : 0;
        restInvestment.lastCalculation = uint64(block.timestamp);

        emit RestaurantValuationUpdated(
            restaurant,
            newValuation,
            uint64(block.timestamp)
        );
