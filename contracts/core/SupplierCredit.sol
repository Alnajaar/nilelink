...
contract SupplierCredit is ISupplierCredit, Ownable, Pausable, ReentrancyGuard {
    ...
    function repay(
        ...
        // Update credit line
        CreditLine storage creditLine = creditLines[invoice.restaurant][
            invoice.supplier
        ];
    ...
    function getSupplierInfo(
        address supplier
    ) external view returns (SupplierInfo memory info) {
    ...
    function getOverdueInvoices(
        address supplier
    ) external pure returns (bytes16[] memory overdueIds) {
    ...
    function disputeInvoice(
        bytes16 invoiceId,
        bytes32 reasonHash
    ) external onlyValidInvoice(invoiceId) {
        Invoice storage invoice = invoices[invoiceId];
    ...
    function _getTermsExpirationDays(
        bytes32 termsHash
    ) internal pure returns (uint64 expirationDays) {
        // Map common terms to expiration periods
        // In production, this would decode the actual terms from the hash
        if (
            termsHash == keccak256(bytes("COD")) ||
            termsHash == keccak256(bytes("Cash on Delivery"))
        ) {
            return 0; // Due immediately
        } else if (
            termsHash == keccak256(bytes("Net-15")) ||
            termsHash == keccak256(bytes("Net15"))
        ) {
            return 15;
        } else if (
            termsHash == keccak256(bytes("Net-30")) ||
            termsHash == keccak256(bytes("Net30"))
        ) {
            return 30;
        } else if (
            termsHash == keccak256(bytes("Net-60")) ||
            termsHash == keccak256(bytes("Net60"))
        ) {
            return 60;
        } else if (
            termsHash == keccak256(bytes("Net-90")) ||
            termsHash == keccak256(bytes("Net90"))
        ) {
            return 90;
        }
